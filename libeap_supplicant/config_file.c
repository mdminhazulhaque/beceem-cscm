...
#ifdef BECEEM_CSCM
static char * wpa_supplicant_conf_pos;
#endif
...
#if BECEEM_CSCM
	int nc;
	pos = wpa_supplicant_conf_pos;
	while (*wpa_supplicant_conf_pos != '\0') {
		for (pos=s, nc=0;
			 *wpa_supplicant_conf_pos != '\n' &&
             *wpa_supplicant_conf_pos != '\0' &&
			 nc < size-2;
			 nc++)
			{
			*pos++ = *wpa_supplicant_conf_pos++;
			}
		if (*wpa_supplicant_conf_pos == '\n') 
			{
			*pos++ = '\n';
			wpa_supplicant_conf_pos++;
			}
						     ...
						     const char *name)
{
	...
	//Debug
	//wpa_hexdump(MSG_MSGDUMP, "wpa_config_read_blob: ", (char *) blob->data, blob->len);
	return blob;
}
...
struct wpa_config * wpa_config_read(const char *name)
{
	...
	if (config == NULL)
		return NULL;
#ifdef BECEEM_CSCM
	wpa_printf(MSG_DEBUG, "Reading configuration directly from string");
	f = NULL;
	wpa_supplicant_conf_pos = (char *) name;
#else
...
#define INT_DEF(t, def) write_int(f, #t, ssid->t, def)
	...
	STR(ca_cert);
	STR(client_cert);
	STR(device_subca1_cert); 
	STR(device_subca2_cert);
	STR(private_key);
