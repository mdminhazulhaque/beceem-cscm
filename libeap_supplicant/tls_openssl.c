...
// Redefine to avoid collisions with new wincrypt.h header from latest MinGW
#define CryptAcquireCertificatePrivateKey localCryptAcquireCertificatePrivateKey
#define CertEnumCertificatesInStore localCertEnumCertificatesInStore
// END CMP fix
...
static int tls_verify_cb(int preverify_ok, X509_STORE_CTX *x509_ctx)
{
	...
	/* Modified by CMP on 27 Nov 2007 
	   Ignores invalid server certificate error
	   Required by Samsung BS at Sprint test site
	*/
	...
	if (!preverify_ok) {
		wpa_printf(MSG_WARNING, "TLS: Certificate verification failed,"
			   ...
			   X509_verify_cert_error_string(err), depth, buf);
		if (err == 26) {
			wpa_printf(MSG_WARNING, "TLS: Ignoring the previous error type 26");
			preverify_ok = 1; 
			}
/*	Modified by:  Raghavan on 26 Mar 2010
	Fix for BUG ID: 13904
	Description: Ignores verification failure due to time check on certificates.
*/		
		else	if (err == X509_V_ERR_ERROR_IN_CERT_NOT_BEFORE_FIELD 
				|| err == X509_V_ERR_CERT_NOT_YET_VALID
				|| err == X509_V_ERR_ERROR_IN_CERT_NOT_AFTER_FIELD
				|| err == X509_V_ERR_CERT_HAS_EXPIRED)
		{
			wpa_printf(MSG_WARNING, "TLS: Ignoring the failure due to time check on certs");
			preverify_ok = 1; 

		}
/* END Modified by:  Raghavan on 26 Mar 2010 */
	} else {
		wpa_printf(MSG_DEBUG, "TLS: tls_verify_cb - "
			   "preverify_ok=%d err=%d (%s) depth=%d buf='%s'",
			   preverify_ok, err,
	...
	/* End CMP modification 27 Nov 2007 */
				  ...
				  size_t ca_cert_blob_len, const char *ca_path)
{
	...
	if (ca_cert_blob) {
		X509 *cert = NULL;
		cert = d2i_X509(NULL, (OPENSSL_d2i_TYPE) &ca_cert_blob,
				      ca_cert_blob_len);
		if (cert == NULL) {
			tls_show_errors(MSG_WARNING, __func__,
				      ...
				      size_t client_cert_blob_len)
{
	if (client_cert == NULL && client_cert_blob == NULL)
		return 0;
/*read the blob as in PEM file.*/
#ifdef BECEEM_CSCM
#ifndef OPENSSL_NO_STDIO
	if(client_cert_blob)
	{
		X509 *cert = NULL;
		BIO *certBuff = BIO_new_mem_buf((u8 *) client_cert_blob,
								client_cert_blob_len);
		if(certBuff)
		{
			cert = PEM_read_bio_X509(certBuff, NULL, 0, NULL);
			BIO_free(certBuff);
			if (cert)
			{
				if(SSL_use_certificate(conn->ssl, cert) == 1)
				{
					wpa_printf(MSG_DEBUG, "OpenSSL: SSL_use_certificate --> "
					   "OK");
					return 0;
				}
				else
					tls_show_errors(MSG_WARNING, __func__, "OpenSSL: SSL_use_certificate --> FAILED");
			}
			else
				tls_show_errors(MSG_WARNING, __func__, "OpenSSL: PEM_read_bio_X509 --> FAILED");
		}
		else
		{
			tls_show_errors(MSG_WARNING, __func__,
...
/*to add extra certs to the cert chain.*/
static int tls_connection_extra_chain_cert(void *ssl_ctx,
				      struct tls_connection *conn,
				      const char *additional_cert,
				      const u8 *additional_cert_blob,
				      size_t additional_cert_blob_len)
{
	BIO *bio = NULL;
	X509 *cert;
	if (additional_cert == NULL && additional_cert_blob == NULL)
	{
		//wpa_printf(MSG_INFO, "OPENSSL %s: Additional cert file not specified.", __func__);
		return 0;
	}

#ifndef OPENSSL_NO_STDIO	
	if(additional_cert_blob)
		bio = BIO_new_mem_buf((u8 *) additional_cert_blob, additional_cert_blob_len);
	else 
		bio = BIO_new_file(additional_cert, "r");
	...
	if (bio != NULL) 
	{
		...
		if (!cert)
		{			
			tls_show_errors(MSG_WARNING, __func__, "OpenSSL: d2i_X509_bio (DER)");
			/*BIO_reset restores BIO to it's original state, 
			for FILE: rewinds the pointer to start of the file.
			for read only MEM: restores the BIO to orignal state.
			for read/write MEM: clears the data in it.
			the present BIO "bio" is a FILE or a Read Only BIO.*/
			BIO_reset(bio);
			cert = PEM_read_bio_X509(bio, NULL, 0, NULL);
			if(!cert)
			{
				tls_show_errors(MSG_WARNING, __func__, "OpenSSL: PEM_read_bio_X509 (PEM)");
				BIO_free(bio);
				return -1;
			}
			else
				wpa_printf(MSG_DEBUG, "OpenSSL: PEM_read_bio_X509 (PEM)"
			   " --> OK");
		}
		else
		{
			wpa_printf(MSG_DEBUG, "OpenSSL: d2i_X509_bio (DER)"
			   " --> OK");
		}
	}
	else
	{
		wpa_printf(MSG_INFO, "TLS: Failed to open additional cert file or blob '%s': %s",
			   additional_cert, ERR_error_string(ERR_get_error(), NULL));
		return -1;
	}
	
	BIO_free(bio);
	if (SSL_CTX_add_extra_chain_cert(ssl_ctx, cert) == 1) {
		wpa_printf(MSG_DEBUG, "OpenSSL: SSL_CTX_add_extra_chain_cert"
			   " extra cert added to chain.");
		return 0;
	} else {
		tls_show_errors(MSG_DEBUG, __func__,
...
#ifndef BECEEM_CSCM
	// Don't check for match between cert and private key, when using Beceem engine
...
/*read the plain private key blob.*/
#ifdef BECEEM_CSCM
		BIO *keyBuff = BIO_new_mem_buf((u8 *) private_key_blob,
								private_key_blob_len);
		if(keyBuff)
		{
			EVP_PKEY *pkey = NULL;
			if ((pkey = PEM_read_bio_PrivateKey(keyBuff, NULL, 0, passwd)) != NULL)
			{
				BIO_free(keyBuff);
				if(	SSL_use_PrivateKey(conn->ssl,pkey) == 1)
				{
					wpa_printf(MSG_DEBUG, "OpenSSL: SSL_use_PrivateKey --> OK");
					ok = 1;
					EVP_PKEY_free(pkey);
					break;
				}
			...
			{
				tls_show_errors(MSG_DEBUG, __func__, "OpenSSL: PEM_read_bio_PrivateKey --> FAILED");
				BIO_free(keyBuff);
			}
		}
		else
			tls_show_errors(MSG_DEBUG, __func__, "OpenSSL: BIO_new_mem_buf FAILED");
#endif
			      ...
			      const struct tls_connection_params *params)
{
	...
	if (tls_connection_client_cert(tls_ctx, conn, params->client_cert,
		...
		return -1;
	if (tls_connection_extra_chain_cert(tls_ctx, conn, params->device_subca1_cert,
 				       					params->device_subca1_cert_blob, 
 				       					params->device_subca1_cert_blob_len))
		return -1;
	if (tls_connection_extra_chain_cert(tls_ctx, conn, params->device_subca2_cert,
				       					params->device_subca2_cert_blob, 
				       					params->device_subca2_cert_blob_len))
		return -1;
	
	/* CMP added 17 Aug 2009 */
	if (params->cipher_rule != NULL)
		SSL_set_cipher_list(conn->ssl, params->cipher_rule);
