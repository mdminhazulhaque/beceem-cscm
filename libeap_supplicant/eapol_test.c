...
static void eapol_sm_cb(struct eapol_sm *eapol, int success, void *ctx)
{
	...
	printf("eapol_sm_cb: success=%d num_reauths=%d\n", 
			  ...
			  int port, const char *secret)
{
	...
	strncpy(e->user_identity, pssid->identity, pssid->identity_len);

	e->user_password = malloc(e->user_password_len);
	assert(e->user_password != NULL);
	strncpy(e->user_password, pssid->password, pssid->password_len);
	// End CMP add
...
__declspec(dllexport) int eapol_wcm_driver(
	char *radius_server_ip_addr,
	int   radius_server_port,
...

{
	...
	int argc = 1;
	char *argv[] = {"wcm_driver"};

	struct wpa_supplicant wpa_s;
	int c, ret = 1, wait_for_monitor = 0, save_config = 0;
	
	char *as_addr = radius_server_ip_addr;
	int as_port = radius_server_port;
	char *as_secret = radius_server_secret;
