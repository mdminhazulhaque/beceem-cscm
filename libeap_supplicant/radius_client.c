				    ...
				    struct radius_msg_list *entry, time_t now)
{
	...
	if (entry->attempts >= RADIUS_CLIENT_MAX_RETRIES) {
		wpa_printf(MSG_ERROR,
			   "Removing un-ACKed RADIUS message due to too many "
...
static void radius_client_receive(int sock, void *eloop_ctx, void *sock_ctx)
{
	...
	if (len == sizeof(buf)) {
		wpa_printf(MSG_ERROR,
			   "Possibly too long UDP frame for our buffer - "
		       "dropping it\n");
		return;
	}
	...
	if (msg == NULL) {
		wpa_printf(MSG_ERROR, "Parsing incoming RADIUS frame failed\n");
...
static int radius_client_init_auth(struct radius_client_data *radius)
{
	...
	if (radius->auth_serv_sock >= 0 &&
	    eloop_register_read_sock(radius->auth_serv_sock,
				     ...
				     (void *) RADIUS_AUTH)) {
		wpa_printf(MSG_ERROR,
				"Could not register read socket for authentication "
		       "server\n");
		return -1;
	}
