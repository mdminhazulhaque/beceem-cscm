			    ...
			    u8 **out_data, size_t *out_len)
{
	...
	u8 *size_pos;
	struct eap_hdr *resp;
	struct eap_ssl_data *ssl_data = &(data->ssl);
	int include_tls_length = ssl_data->include_tls_length;

	/* TODO: add support for fragmentation, if needed. This will need to
	 * add TLS Message Length field, if the frame is fragmented. */
	//resp = malloc(sizeof(struct eap_hdr) + 2 + data->ssl.tls_out_limit);
	resp = malloc(sizeof(struct eap_hdr) + 2 + 4 + data->ssl.tls_out_limit);
	if (resp == NULL)
		return -1;
	...
	*pos++ = EAP_TYPE_TTLS;
	...
	if (include_tls_length)
		{
		wpa_printf(MSG_DEBUG, "SSL: Msg length include succeeded");
		*flags |= EAP_TLS_FLAGS_LENGTH_INCLUDED;
		*size_pos++ = (res >> 24) & 0xff;
		*size_pos++ = (res >> 16) & 0xff;
		*size_pos++ = (res >>  8) & 0xff;
		*size_pos++ = (res      ) & 0xff;	
		*out_len = sizeof(struct eap_hdr) + 2 + 4 + res;
		}
	else
...
static u8 * eap_ttls_getKey(struct eap_sm *sm, void *priv, size_t *len)
{
	...
	key = malloc(EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
	if (key == NULL)
		return NULL;

	*len = EAP_TLS_KEY_LEN + EAP_EMSK_LEN;
	memcpy(key, data->key_data, EAP_TLS_KEY_LEN + EAP_EMSK_LEN);
